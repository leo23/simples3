...
def _oss_canonicalize(headers):
    r"""Canonicalize OSS headers in that certain OSS way.

    >>> _oss_canonicalize({"x-oss-test": "test"})
    'x-oss-test:test\n'
    >>> _oss_canonicalize({"x-oss-first": "test",
    ...                    "x-oss-second": "hello"})
    'x-oss-first:test\nx-oss-second:hello\n'
    >>> _oss_canonicalize({})
    ''
...
def metadata_headers(metadata):
    return dict(("x-oss-meta-" + h, v) for h, v in metadata.iteritems())

def headers_metadata(headers):
    return dict((h[11:], v) for h, v in headers.iteritems()
                            if h.lower().startswith("x-oss-meta-"))
...
def oss_md5(data):
    """Make an OSS-style MD5 hash (digest in base64)."""
...
def oss_urlquote(value):
    r"""OSS-style quote a URL part.

    >>> oss_urlquote("/bucket/a key")
    '/bucket/a%20key'
    """
...
def name(o):
    ...
    Functions:
    >>> name(name)
    'simpleoss.utils.name'
    >>> def my_fun(): pass
    >>> name(my_fun)
    'simpleoss.utils.my_fun'

    Classes:
    >>> class MyKlass(object): pass
    >>> name(MyKlass)
    'simpleoss.utils.MyKlass'
